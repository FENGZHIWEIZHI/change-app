<template>
	<view>
		...
		<u-card v-for="(item,index) in orderList" :key="item.id">
			...
			<view slot="body">
				<view>
					<view class="u-margin-top-10 u-flex">
						<text>就诊人：{{item.patientName}}</text>
						<text class="u-m-l-80">挂号费： ¥{{item.total}}</text>
					</view>
					<view class="u-margin-top-10">就诊日期：{{item.periodDate}} ({{item.weekDay}}) {{item.periodTime}}</view>
					<view class="u-margin-top-10">就诊医院：{{item.hospitalName}}</view>
					<view class="u-margin-top-10">医院地址：{{item.hospitalAddress}}</view>
				</view>
			</view>
			<view slot="foot">
				<view class="u-flex u-row-between">
					...
					<view v-if="item.status=='已支付' || item.status=='已取消'">
						<view>
							<u-button type="primary" shape="circle" :plain="true" size="mini" @click.stop="deleteOrder(item.id, index)">删除订单
							</u-button>
						</view>
					</view>
					<view v-if="item.status=='待支付'">
						<u-button type="primary" shape="circle" :plain="true" size="mini" @click.stop="cancelOrder(item.id, index)">取消订单
						</u-button>
...
<script>
	export default {
		...
		methods: {
			...
			getOrderList() {
				this.$u.api.listReservation(this.queryParams).then((res) => {
					let list = res.rows;
					list.forEach(item => {
			...
			getStatusOpts() {
				this.$u.api.getDicts("reservation_order_status").then(response => {
			...
			// 处理删除逻辑
			handleDel() {
				this.$u.api.delReservation(this.selectId).then(response => {
